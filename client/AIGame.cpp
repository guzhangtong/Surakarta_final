...
AIGame::AIGame(MoveStageMessage *pmsg, QWidget *parent)
    : Board(pmsg, parent)
    , pmoveStageMsg(pmsg)
{
    _level = 4;
    connect(this, &Board::sig_userChanged,this,&AIGame::userChanged);
}
...
void AIGame::aiRunTimeOut()
{
    copyStone(_s_back, _s);
    // qDebug() << "ai agent operation";
    QThread::msleep(2000);//sleep 0.5s;
    AIgo();
    int dead1=0,dead2=0;
    for(int i=0;i<24;i++)
    {
...
void AIGame::userChanged(bool user)
{
    if (ai_mode[bTurn] && user != bTurn)
    {
        QTimer::singleShot(500, this, &AIGame::aiRunTimeOut);//sleep 0.5s;
        // copyStone(_s_back, _s);
        // // qDebug() << "ai agent operation";
        // AIgo();
        // int dead1=0,dead2=0;
...
void AIGame::mouseReleaseEvent(QMouseEvent *ev) {
    ...
    {
         if(ai_mode[bTurn] != false )
        {
            //only human can click mouse;
            canClick = false;
        }
    }
    ...
    if(canClick)
    {
        //only human can click mouse;
        copyStone(_s_back, _s);
        SOCKET_INDEX index = ai_mode[bTurn] == true ? AISocket_Index : HumenSocket_Index;
        Board::click(ev->pos(), index);
    }
}
...
void AIGame::AIgo()
{
    ...
    if(step != nullptr)
    {
        saveStep(step->_moveid, step->_killid, step->_rowTo, step->_colTo, _steps);

        QPoint fromPt = this->center(step->_rowFrom, step->_colFrom);
        QPoint toPt = this->center(step->_rowTo, step->_colTo);

        this->click(fromPt, AISocket_Index);
        this->click(toPt, AISocket_Index);
    }
