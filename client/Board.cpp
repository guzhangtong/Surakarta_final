...
int __row = _s[__id]._x;\
    int __col = _s[__id]._y
      ...
      Board::Board(MoveStageMessage *pmsg, QWidget *parent):QWidget(parent)
, pmoveStageMsg(pmsg)
, bTurn(false)
{
    mapStepRecord.insert(false, QVector<StepRecord>());
    mapStepRecord.insert(true, QVector<StepRecord>());
    mapThinkTime.insert(false, 0);
    mapThinkTime.insert(true, 0);
    init();
}
...
QString Board::EndGameResult(bool isWhiteWin)
{
    QString result;
    result += QString("胜利方:") + (isWhiteWin==true ? QString("白棋") : QString("黑棋")) + QString("\n");

    int count = 0;
    for(auto & it : this->_s)
    {
        if(it._color == isWhiteWin)
        {
            if(it._dead == false)
            {
                count++;
    ...
    result += QString("净胜子:") + QString::number(count) + QString("\n");
    result += QString("步数:") + QString::number(this->mapStepRecord[isWhiteWin].size()) + QString("\n");

    int averageThinkTime = 0;
    for(auto&it : this->mapStepRecord[isWhiteWin])
    {
        averageThinkTime += it.ThinkTime;
    }
    averageThinkTime = averageThinkTime / this->mapStepRecord[isWhiteWin].size();
    result += QString("平均思考时间:") + QString::number(averageThinkTime) + QString("秒") + QString("\n");

    return result;
}

void Board::SetThinkTime(bool isWhite, int time)
{
    this->mapThinkTime[isWhite] = time;
}

void Board::AddStepRecode(bool isWhite)
{
    StepRecord record;
    record.ThinkTime = this->mapThinkTime[isWhite];
    this->mapStepRecord[isWhite].push_back(record);
}
...
void Board::click(QPoint pt, SOCKET_INDEX index)
{
    ...
    for( i=0;i<24;i++)
    {
        ...
        {
            clickid=i;
            if(this->pmoveStageMsg != nullptr && this->_select < 0)
            {
                //set click point;
                this->pmoveStageMsg->saveClickStonePoint(pt);
                this->pmoveStageMsg->saveMoveFromPoint(pt);
            }
            break;
        }
    ...
    if(this->_select != -1)
    {
        if(this->pmoveStageMsg!= nullptr)
            this->pmoveStageMsg->saveMoveToPoint(pt);
    }
    click(clickid,x,y, index);
}
void Board::click(int clickid, int x, int y, SOCKET_INDEX index){
    if(clickid!=-1&&bTurn==_s[clickid]._color)
    {
        // if(this->pmoveStageMsg != nullptr)
        // {
        //     //click on the right stone, send message;
        //     this->pmoveStageMsg->clickStone();
        //     this->pmoveStageMsg->SendMessage(index);
        // }
        int flag=0;
    ...
    else if( _select!=-1)
    {
        if(canMovel(_select,x,y,clickid)==true)
        {
            ...
            if(this->pmoveStageMsg != nullptr)
            {
                //can move stone, send message;
                this->pmoveStageMsg->moveStone();
                this->pmoveStageMsg->SendMessage(index);
            }
            ...
            _select=-1;
            // bTurn=!bTurn;
            bool srcTurn = this->bTurn;
            this->AddStepRecode(bTurn);
           // if(this->pmoveStageMsg == nullptr)
           // {
                //online, turn by server
                this->setTurn(!bTurn);
            //}
...
bool Board::isCanMoveTo(QPoint ptFrom, QPoint ptTo)
{
    int rowFrom = -1;
    int colFrom = -1;
    int rowTo = -1;
    int colTo = -1;
    bool yesFrom = this->getRowCol(ptFrom, rowFrom, colFrom);
    bool yesTo = this->getRowCol(ptTo, rowTo, colTo);
    if(!yesFrom || !yesTo)
    {
        return false;
    }
    ...
    for(int i=0; i < 24; i++)
    {
        if(this->_s[i]._x == rowTo && this->_s[i]._y == colTo)
        {
            clickId = i;
            break;
        }
    ...
    if(clickId >=0 )
    {
        if(this->_s[clickId]._color == this->bTurn)
            isMove = false;
        //no finish;
    }
    else
    {
        if(qAbs(rowFrom-rowTo)<=1 && qAbs(colFrom-colTo)<=1 )
        {
            isMove =  true;
        }
        else
...
void Board::reset()
{
    init();
    update();
    emit sig_userChanged(false);//reset game,send the signa be the user change to white user player;
}
