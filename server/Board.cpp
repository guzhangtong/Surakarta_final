...
int __row = _s[__id]._x;\
    int __col = _s[__id]._y
      ...
      Board::Board(MoveStageMessage*pmoveStageMsg, EndStageMessage *pendStageMsg, QWidget *parent):QWidget(parent)
, pmoveStageMessage(pmoveStageMsg)
, pendStageMessage(pendStageMsg)
{
    mapThinkTime.insert(false, THINK_TIME);
    mapThinkTime.insert(true, THINK_TIME);
    connect(&this->thinkPlayTimer, &QTimer::timeout, this, &Board::ThinkTimeTimeOut);
    connect(this, &Board::sig_finish, this, &Board::GameFinishSlot);
    init();
}
...
void Board::setTurn(bool b)
{
    bool isTurn = bTurn != b;
    if(isTurn)
    {
        this->mapThinkTime[false] = THINK_TIME;
        this->mapThinkTime[true] = THINK_TIME;
    }
    bTurn = b;
   // if(isTurn)
   // {
        //QTimer::singleShot(200, this, &Board::SyncTurnTimeOut);
        // if(this->pmoveStageMessage != nullptr)
        // {
        //     this->pmoveStageMessage->turnToWho(this->bTurn);
        //     QTcpSocket* psocket = this->pmoveStageMessage->GetSocket(this->bTurn);
        //     QTcpSocket* psocketOther = this->pmoveStageMessage->GetSocket(!this->bTurn);
        //     this->pmoveStageMessage->SendMessage(psocket);
        //     this->pmoveStageMessage->SendMessage(psocketOther);
        // }
   // }
}

void Board::ThinkTimeTimeOut()
{
    this->mapThinkTime[this->bTurn] = this->mapThinkTime[this->bTurn] <= 0 ? 0 : this->mapThinkTime[bTurn] - 1;
    if(this->pmoveStageMessage != nullptr)
    {
        this->pmoveStageMessage->thinkTimerToWho(this->bTurn, this->mapThinkTime[this->bTurn]);
        QTcpSocket* psocket = this->pmoveStageMessage->GetSocket(this->bTurn);
        QTcpSocket* psocketOther = this->pmoveStageMessage->GetSocket(!this->bTurn);
        this->pmoveStageMessage->SendMessage(psocket);
        this->pmoveStageMessage->SendMessage(psocketOther);
    ...
    if(this->mapThinkTime[this->bTurn] <= 0)
    {
        this->GameFinish(this->bTurn, SurakartaEndReason::TIMEOUT);
    }
}

void Board::SyncTurnTimeOut()
{
    if(this->pmoveStageMessage != nullptr)
    {
        this->pmoveStageMessage->turnToWho(this->bTurn);
...
void Board::GameFinish(bool isWhiteLose, SurakartaEndReason reason)
{
    if(this->thinkPlayTimer.isActive())
        this->thinkPlayTimer.stop();

    if(this->pendStageMessage != nullptr )
    {
        if(reason == SurakartaEndReason::TIMEOUT)
        {
            this->pendStageMessage->TimeoutEndGame(isWhiteLose);
            QTcpSocket* psocket1 = this->pendStageMessage->GetSocket(isWhiteLose);
            QTcpSocket* psocket2 = this->pendStageMessage->GetSocket(!isWhiteLose);
            //both send end message
            this->pendStageMessage->SendMessage(psocket1);
            this->pendStageMessage->SendMessage(psocket2);
        }

        if(reason == SurakartaEndReason::RESIGN)
        {
            this->pendStageMessage->EndGameReason(isWhiteLose, reason);
            QTcpSocket* psocket1 = this->pendStageMessage->GetSocket(isWhiteLose);
            QTcpSocket* psocket2 = this->pendStageMessage->GetSocket(!isWhiteLose);
            //both send end message
            this->pendStageMessage->SendMessage(psocket1);
...
void Board::GameFinishSlot(bool isWhiteLose)
{
    if(this->thinkPlayTimer.isActive())
        this->thinkPlayTimer.stop();
    ...
    if(this->pendStageMessage != nullptr )
    {
        SurakartaEndReason reason = SurakartaEndReason::CHECKMATE;
...
bool Board::click(QPoint pt)
{
    ...
    return click(clickid,x,y);
}
bool Board::click(int clickid, int x, int y){
    ...
    if(clickid!=-1&&bTurn==_s[clickid]._color)
    {
        ...
        for(int i=0;i<=5;i++)//CAN GO TO
        {
            for(int j=0;j<5;j++)
            {
                ...
                if((isCanMove=canMovel(clickid,i,j,toid))==true)
                {
                    can_go_x[flag]=i;
                    can_go_y[flag]=j;
                    flag++;
    ...
    {
        if((isCanMove=canMovel(_select,x,y,clickid))==true)
        {
            /*判断能否走棋子*/
            _s[_select]._x=x;
            _s[_select]._y=y;
...
void Board::init()
{
    ...
    if(!thinkPlayTimer.isActive())
    {                
        thinkPlayTimer.start(1*1000);
    }
}
